...
Keys live only in that user's Streamlit session (st.session_state) — never
written to disk, never shared across users, never hardcoded.
"""
...
def render_api_key_sidebar() -> None:
    with st.sidebar:
        ...
        st.markdown("---")
        st.caption("Get a free VirusTotal key at virustotal.com (Profile → API Key).")
        st.caption("Get a free Gemini key at aistudio.google.com/apikey.")
...
def main() -> None:
    ...
    if not st.session_state.get("VT_API_KEY") or not st.session_state.get("GEMINI_API_KEY"):
        st.error("Please enter both your VirusTotal and Gemini API keys in the sidebar first.")
        return
